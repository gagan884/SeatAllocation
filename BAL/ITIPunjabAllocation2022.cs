...
namespace BAL
{
    public class ITIPunjabAllocation2022 : ComCouns21Allocation
    {
        public ITIPunjabAllocation2022() : base()
        {
            // board Id 
            boardId = "125012221";
        ...
        string DBProc_GetCandidateEligibleProgram = "Get_CandidateEligibleProgram";
        public override ActionOutput
            ...
            CreateVirtualChoice()
        {
            ...
            string candGendervalue = string.Empty;
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                while ((rdrChoice.Read()))
                {
                    ...
                    if (isValidChoice)
                    {
                        if (!StreamInfoMaster.ContainsKey(choiceInstCd + "." + choiceBrCd))
                        {
                            continue; ;
                        }
                        if (CandidateEligibleProgram.ContainsKey(candRollNo) && !CandidateEligibleProgram.ContainsValue(choiceBrCd))
                        {
                            continue;
                        }
                        choiceStreamId = StreamInfoMaster[choiceInstCd + "." + choiceBrCd]; /*Add only those which exist in seat */
                        choiceGroupId = GroupMaster[choiceInstCd + "." + choiceBrCd];
                        choiceSeatTypeId = SeatTypeMaster[choiceInstCd + "." + choiceBrCd];
                        if (candDomicile.Equals("28"))
                            choiceQuotaId = "HS";
                        else
                            choiceQuotaId = "OS";
                        ...
                        {
                            waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + "." + choiceSeatTypeId + "." + choiceQuotaId + "." + option;
        ...
        protected Dictionary<string, string> CandidateEligibleProgram = null;
        protected void LoadCandidateEligibleProgram()
        {
            CandidateEligibleProgram = new Dictionary<string, string>();
            SqlParameter[] sqlParameters = new SqlParameter[1];
            sqlParameters[0] = new SqlParameter("@" + DBParam_BoardId, boardId);
            DataTable dtCandEligibleProgram = SqlHelper.ExecuteDataset(connectionString, DBProc_GetCandidateEligibleProgram, sqlParameters).Tables[0];
            foreach (DataRow dr in dtCandEligibleProgram.Rows)
            {
                CandidateEligibleProgram.Add(dr["rollNo"].ToString(), dr["programs"].ToString());
            }
        }
        ...
        protected override string GetAllotmentSequence(string rollno, string category, string subcategoryList, string gender, string domicile, string additionalInfo, string symbolList, string rankData)
        {
            ...
            {
                if (gender.Equals("02"))
                    catSubCatOptions.Append("OP.NO.F,");

                catSubCatOptions.Append("OP.NO.B,");
                ...
                if (!category.Equals("GN"))
                {
                    if (gender.Equals("02"))
                    {
                        ...
                        {
                            catSubCatOptions.Append("01.NO.F,");
                        }
                        if (category.Equals("11") || category.Equals("12"))
                        {
                            catSubCatOptions.Append("13.NO.F,");
                        }
                        catSubCatOptions.Append(category + ".NO.F,");
                    }
                    if (category.Equals("SC") || category.Equals("ST"))
                    {
                        catSubCatOptions.Append("01.NO.B,");
                    }
                    if (category.Equals("11") || category.Equals("12"))
                    {
                        catSubCatOptions.Append("13.NO.B,");
                    }
                    catSubCatOptions.Append(category + ".NO.B,");
                }
            }
