...
namespace BAL
{
    public class ShreshtaAllocation2022 : ComCouns21Allocation
    {
        public ShreshtaAllocation2022() : base()
        {
            // board Id 
            boardId = "143012221";
            ...
            //DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_Shreshta";/*Changed on 03June2022*/
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_Shreshta";/*Changed on 14June2022*/

            //Procedure to prepare Previous allotment or seat gaurnatee candidates
            DBProc_PreparePreviousAllotment = "XApp_CC_PreparePreviousAllotment_Shreshta";
        ...
        public override ActionOutput CreateVirtualChoice()
        {
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                candGendervalue = drCand["AdditionalInfo"].ToString().Split('=')[1];
                ...
                while ((rdrChoice.Read()))
                {
                    ...
                    if (isValidChoice)
                    {
                        ...
                        foreach (string option in catSubCatOptions.Split(','))
                        {
                            ...
                            {
                                if(genderid.ToUpper() == candGendervalue.ToUpper() || genderid.ToUpper() == "B" )
                                {
                                    waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + "." + choiceSeatTypeId + "." + choiceQuotaId + "." + option + "." + genderid;
        ...
        protected override string GetAllotmentSequence(string rollno, string category, string subcategoryList, string gender, string domicile, string additionalInfo, string symbolList, string rankData)
        {
            ...
            catSubCatOptions.Append("SC.NO,"); /*Only Category is SC and no Subcategory*/
