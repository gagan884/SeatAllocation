...
namespace BAL
{
    public class HBTUBTechAllocation2022 : ComCouns21Allocation
    {
        public HBTUBTechAllocation2022() : base()
        {
            // board Id 
            boardId = "131012221";

            //Procedure to prepare seat
            DBProc_PrepareSeat = "XApp_CC_PrepareSeat_HBTU";

            //Procedure to prepare Eligible Candidate
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_HBTU";

            //Procedure to prepare Previous allotment or seat gaurnatee candidates
            DBProc_PreparePreviousAllotment = "XApp_CC_PreparePreviousAllotment_HBTU";

            //Procedure to dereserve seats
            DBProc_DereserveSeat = "XApp_CC_DereserveSeat_HBTU";
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                while ((rdrChoice.Read()))
                {
                    ...
                    if (isValidChoice)
                    {
                        ...
                        if (roundNo == 4 || roundNo == 6)
                        {
                            if (drCand["Willingness"].ToString() == "FR")
                            {
                                waitListKey = choiceInstCd + "." + choiceBrCd + "." + objAllAllotmentDetails[candRollNo].Sequence;
                                rank = candRanks[SeatRankTypeMapping[waitListKey]];
                                procOptno = procOptno + 1;
                                choices = choices.Append(choiceInstCd + "." + choiceBrCd + "." + objAllAllotmentDetails[candRollNo].Sequence + ":" + rank.ToString() + ",");
                                continue;
                            }
                        }
        ...
        protected override string GetAllotmentSequence(string rollno, string category, string subcategoryList, string gender, string domicile, string additionalInfo, string symbolList, string rankData)
        {
            ...
            bool isUPDomicile = (!domicile.Equals("98") || !stateId.Equals("98")) ? true : false;
            ...
            if ("01,02".Contains(gender))
            {
                if (isUPDomicile)
                {
                    ...
                    foreach (string sub in subcategoryList.Split(','))
                    {
                        ...
                        {
                            catSubCatOptions.Append("HS.OP." + sub.Substring(0, 2) + ",");
                        }
                    }
