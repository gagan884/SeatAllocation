...
namespace BAL
{
    public class ANMGNMAllocation2022 : ComCouns21Allocation
    {
        public ANMGNMAllocation2022() : base()
        {
            // board Id 
            boardId = "134172221";
            ...
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_ANMGNM";

            //Procedure to prepare Previous allotment or seat gaurnatee candidates
            DBProc_PreparePreviousAllotment = "XApp_CC_PreparePreviousAllotment_ANMGNM";
                ...
                while ((rdrChoice.Read()))
                {
                    ...
                    if (isValidChoice)
                    {
                        ...
                        foreach (string option in catSubCatOptions.Split(','))
                        {
                            ...
                            //if (SeatRankTypeMapping.ContainsKey(waitListKey) && candRanks.ContainsKey("59.01")
                            /*Rank Type ID Changed by Shivam on 23 Aug as per current year*/
                            if (SeatRankTypeMapping.ContainsKey(waitListKey) && candRanks.ContainsKey("59.12"))
                            {
                                //choiceRankType = "59.01";
                                choiceRankType = "59.12";
        ...
        protected override string GetAllotmentSequence(string rollno, string category, string subcategoryList, string gender, string domicile, string additionalInfo, string symbolList, string rankData)
        {
            ...
            if (subcategoryList.Contains("OH:YES"))
            {
                catSubCatOptions.Append("NO.OH,");
            }


            if (subcategoryList.Contains("CD:YES"))
            {
                catSubCatOptions.Append("NO.CD,");
            }
