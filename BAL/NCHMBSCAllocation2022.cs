...
namespace BAL
{
    public class NCHMBSCAllocation2022 : ComCouns21Allocation
    {
        public NCHMBSCAllocation2022() : base()
        {
            // board Id 
            boardId = "103012221"; /*Change BoardId for current counselling*/

            //Procedure to prepare seat
            DBProc_PrepareSeat = "XApp_CC_PrepareSeat_NCHM";

            //Procedure to prepare Eligible Candidate
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_NCHMBSC";

            //Procedure to prepare Previous allotment or seat gaurnatee candidates
            DBProc_PreparePreviousAllotment = "XApp_CC_PreparePreviousAllotment_NCHM";

            //Procedure to dereserve seats
            DBProc_DereserveSeat = "XApp_CC_DereserveSeat_NCHM";
            ...
            DBProc_GetSeatRankTypeMapping = "XApp_CC_GetSeatRankTypeMapping_NCHM";

            //Procedure to get mapping between institute-program and Stream
            DBProc_GetStreamMapping = "XApp_CC_InstituteProgramStreamMaster_NCHM";

            //Procedure to get mapping between institute-program and seat type
            DBProc_GetSeatTypeMapping = "XApp_CC_InstituteProgramSeatTypeMaster_NCHM";

            //Procedure to get mapping between institute-program and group type
            DBProc_GetGroupMapping = "XApp_CC_InstituteProgramGroupMaster_NHCM";

            //Procedure to get mapping between institute-program and Quota
            DBProc_GetQuotaMapping = "XApp_CC_InstituteProgramQuotaMaster_NCHM";

            //Procedure to get mapping between institute-program and gender
            DBProc_GetGenderMapping = "XApp_CC_InstituteProgramGenderMaster_NCHM";
            ...
            DBProc_GetCandidateChoice = "XApp_CC_GetCandidateChoice_NCHM";            
        ...
        //                {
        //                    if (choiceBrCd.EndsWith("T") && !option.Equals("OP.TF"))
        //                    {
        //                        continue;
        //                    }
        //                    else if (!choiceBrCd.EndsWith("T") && option.Equals("OP.TF"))
        //                    {
        //                        continue;
        //                    }
        //                    if (roundNo <= 4)
        ...
        //                        if (candGender == "01")
        //                        {
        //                            waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + "." + choiceSeatTypeId + "." + choiceQuotaId + "." + option + "." + choiceGenderId.Split(',')[1];

        //                        }
        //                        else
        //                        {
        //                            waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + "." + choiceSeatTypeId + "." + choiceQuotaId + "." + option + "." + choiceGenderId.Split(',')[0];
        ...
        public override ActionOutput CreateVirtualChoice()
        {
            if (roundNo <= 4)
            {
                return base.CreateVirtualChoice();
            }
            else
            {
                LoadSeatDetails();
                ...
                foreach (DataRow drCand in dtCand.Rows)
                {
                    ...
                    while ((rdrChoice.Read()))
                    {
                        ...
                        foreach (string option in catSubCatOptions.Split(','))
                        {
                            choiceGenderId = candGender == "01" ? "M" : "F";
                            waitListKey = choiceInstCd + "." + choiceBrCd + ".21.G1.NCHM.AI." + option + "." + choiceGenderId;
                            rank = candRanks["21.01"];
                            procOptno = procOptno + 1;
                            choices = choices.Append(waitListKey + ":" + rank.ToString() + ",");
                        }
                    }
        ...
        protected override string GetAllotmentSequence(string rollno, string category, string subcategoryList, string gender, string domicile, string additionalInfo, string symbolList, string rankData)
        {
            ...
            if (subcategoryList.Contains("KM:YES"))
            {
                catSubCatOptions.Append("NO.KM,");
            }
