...
namespace BAL
{
    public class AYUSHPGAllocation : ComCouns21Allocation
    {
        public AYUSHPGAllocation() : base()
        {
            // board Id 
            boardId = "141022121";
            ...
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_AYUSHPG";

            //Procedure to prepare Previous allotment or seat gaurnatee candidates
            DBProc_PreparePreviousAllotment = "XApp_CC_PreparePreviousAllotment_AYUSHPG";

            //Procedure to dereserve seats
            DBProc_DereserveSeat = "XApp_CC_DereserveSeat_AYUSHPG";

            //Procedure to update the allotment,allotment summary and other output table
            //after completion of allocation
            DBProc_UpdateApplication = "XApp_CC_UpdateApplication_AYUSHPG";
            ...
            DBProc_GetCandidateChoice = "XApp_CC_GetCandidateChoice_AYUSHPG";
        ...
        public override ActionOutput CreateVirtualChoice()
        {
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                // this.options = this.GetAllotmentOptionCode(drCand["Domicile"].ToString(), drCand["AdditionalInfo"].ToString().Split(new char[] { '=' })[1], drCand["Category"].ToString(), drCand["Subcategory"].ToString(), drCand["Symbol"].ToString());
                //this.options = this.GetAllotmentOptionCode(drCand["Category"].ToString(), drCand["Subcategory"].ToString(), drCand["Symbol"].ToString());
                ...
                while (rdrChoice.Read())
                {
                    ...
                    if (this.objAllAllotmentDetails.ContainsKey(drCand["Rollno"].ToString()) && this.objAllAllotmentDetails[drCand["Rollno"].ToString()].IsRetained != 0)
                    {
                        isRetained = this.objAllAllotmentDetails[drCand["Rollno"].ToString()].IsRetained;
                        Optno = this.objAllAllotmentDetails[drCand["Rollno"].ToString()].OptNo;
                    }
                    if (isRetained == 0)
                    {
                        isValidChoice = true;
                    ...
                    this.options = this.GetAllotmentOptionCode(drCand["Category"].ToString(), drCand["Subcategory"].ToString(), drCand["Symbol"].ToString(), rdrChoice["QuotaId"].ToString());

                    rank = Convert.ToDouble(drCand["Rank"].ToString().Split(new char[] { '=' })[1]);

                    string[] strArrays = this.options.Split(new char[] { ';' });
                    for (int i = 0; i < (int)strArrays.Length; i++)
                    {
                        string option = strArrays[i];
                        procOptno++;

                        choices = string.Concat(new object[] { choices, rdrChoice["Counsid"].ToString(), ".", rdrChoice["QuotaId"].ToString(), ".", rdrChoice["Instcd"].ToString().Trim(), ".", rdrChoice["Brcd"].ToString().Trim(), ".", option, ":", rank, "," });
                    }
        ...
        */
        ...
        public override ActionOutput CreateVirtualChoice()
        {
            ...
            LoadGroupInfo();
            LoadQuotaInfo();
            //return new ActionOutput(ActionStatus.Success, "Completed:Choice created for  candidates");
            DataTable dtAllChoices = new DataTable();
            dtAllChoices.Columns.Add("rollNo");
            dtAllChoices.Columns.Add("totalChoices");
            dtAllChoices.Columns.Add("totalVChoices");
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                while ((rdrChoice.Read()))
                {
                    ...
                    if (isValidChoice)
                    {
                        ...
                        if (objAllAllotmentDetails.ContainsKey(candRollNo) && objAllAllotmentDetails[candRollNo].Instcd.Trim() == choiceInstCd && objAllAllotmentDetails[candRollNo].Brcd.Trim() == choiceBrCd && objAllAllotmentDetails[candRollNo].Sequence.Substring(9, 2) == choiceQuotaId && choiceStreamId == objAllAllotmentDetails[candRollNo].Sequence.Substring(0, 2))
                        {
