...
namespace BAL
{
    public class CCMNAllocation2022 : AbstractAllocation
    {
        ...
        public CCMNAllocation2022() : base()
        {

            // board Id 
            boardId = "138012221"; /*BoardId changed and refered from CCMT2021 counselling db*/

            //Procedure to prepare seat
            DBProc_PrepareSeat = "XApp_CC_PrepareSeat";

            //Procedure to prepare Eligible Candidate
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_CCMN";
            ...
            DBProc_GetCandidateChoice = "XApp_CC_GetCandidateChoice_CCMN";            
        ...
        public override ActionOutput CreateVirtualChoice()
        {
            ...
            cmdChoice.Parameters.Add("@" + DBParam_BoardId, SqlDbType.VarChar);
            cmdChoice.Parameters.Add("@" + DBParam_GateCd, SqlDbType.VarChar);
            cmdChoice.Parameters.Add("@" + DBParam_QDegreeType, SqlDbType.VarChar);
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                //if (candRollNo == "CE20S71401732") /*Added Only for Test purpose during Debugging*/
                //{
                //    ;
                //}
        ...
        /*New Fn added by shivam from webapp*/
        private Dictionary<string, int> SurrendredCandidates;
        //private void LoadSurrenderedCandidates()
        //{
        //    SurrendredCandidates = new Dictionary<string, int>();
        //    string query = string.Format("select rollNo, optionNo from App_Allotment A where boardId = {0} and roundNo<{1} and exists(select 1 from App_Willingness where boardid = {0} and rollno = A.rollNo  and roundno=A.roundNo and Willingness = 'RF')", boardId, roundNo);
        //    DataTable dt = SqlHelper.ExecuteDataset(connectionString, CommandType.Text, query).Tables[0];
        //    if (dt != null && dt.Rows.Count > 0)
        //    {
        //        foreach (DataRow dr in dt.Rows)
        //        {
        //            SurrendredCandidates.Add(dr["rollNo"].ToString(), Convert.ToInt32(dr["optionNo"]));
        //        }
        //    }
        //}
        ...
        //    cmdChoice.CommandType = CommandType.StoredProcedure;
        //    cmdChoice.Parameters.Add("@QDegreeType", SqlDbType.VarChar);
        //    cmdChoice.Parameters.Add("@Degree", SqlDbType.VarChar);
        //    cmdChoice.Parameters.Add("@GateCd", SqlDbType.VarChar);
        //    cmdChoice.Parameters.Add("@RollNo", SqlDbType.VarChar);
        //    cmdChoice.Parameters.Add("@boardId", SqlDbType.VarChar);
        //    SqlDataReader rdrChoice = default(SqlDataReader);
        //    connChoice.Open();

        //    DataTable dtCand = SqlHelper.ExecuteDataset(connectionString, DBProc_GetEligibleCandidate).Tables[0];

        //    List<string> ExceptionalCandidates = new List<string>();
        ...
        //        options = GetAllotmentOptionCode(drCand["Symbol"].ToString(), drCand["Category"].ToString(), drCand["Subcategory"].ToString());
        //        if (string.IsNullOrEmpty(options))
        //            continue;
        //        cmdChoice.Parameters["@QDegreeType"].SqlValue = drCand["AdditionalInfo"].ToString().Split('|')[0].Split('=')[1];
        //        cmdChoice.Parameters["@Degree"].SqlValue = drCand["AdditionalInfo"].ToString().Split('|')[2].Split('=')[1];
        //        cmdChoice.Parameters["@GateCd"].SqlValue = drCand["AdditionalInfo"].ToString().Split('|')[1].Split('=')[1];
        //        cmdChoice.Parameters["@RollNo"].SqlValue = drCand["Rollno"].ToString();
        //        cmdChoice.Parameters["@boardId"].SqlValue = boardId;
        //        rdrChoice = cmdChoice.ExecuteReader();
        //        while ((rdrChoice.Read()))
        //        {
        //            isRetained = 0;
        ...
        //            if (isRetained == 0)
        //            {
        //                if (drCand["Willingness"].ToString().Equals("RF") && SurrendredCandidates.ContainsKey(drCand["Rollno"].ToString()))
        //                {
        //                    isValidChoice = (SurrendredCandidates[drCand["Rollno"].ToString()] > Convert.ToInt32(rdrChoice["optno"]));
        //                }
        //                else
        //                {
        //                    isValidChoice = true;
        ...
        //                {
        //                    rank = Convert.ToDouble(drCand["Rank"]);
        //                    procOptno = procOptno + 1;
        //                    choices = choices + (rdrChoice["instcd"].ToString() + "." + rdrChoice["brcd"].ToString() + "." + option + rdrChoice["GroupCd"].ToString().Trim() + ":" + rank.ToString() + ",");
        //                }
        //            }
        //        }
        //        rdrChoice.Close();
