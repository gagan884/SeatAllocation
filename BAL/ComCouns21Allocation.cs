...
namespace BAL
{
    public class ComCouns21Allocation : AbstractAllocation
    {
        ...
        protected virtual string GetAllotmentSequenceFromDB(string rollno, string category, string subcategoryList, string gender, string domicile, string additionalInfo, string symbol, string rankData)
        {
            try
            {
                ...
                sqlParameters[4] = new SqlParameter("@" + DBParam_CandGender, gender);
                sqlParameters[5] = new SqlParameter("@" + DBParam_CandDomicile, domicile);
                sqlParameters[6] = new SqlParameter("@" + DBParam_CandAddionalInfo, additionalInfo);
                sqlParameters[7] = new SqlParameter("@" + DBParam_CandSymbol, symbol);
                sqlParameters[8] = new SqlParameter("@" + DBParam_CandRank, rankData);
                sqlParameters[9] = new SqlParameter("@" + DBParam_result, SqlDbType.Int) { Direction = ParameterDirection.Output };
                sqlParameters[10] = new SqlParameter("@" + DBParam_result, SqlDbType.VarChar, 500) { Direction = ParameterDirection.Output };
                int status = SqlHelper.ExecuteNonQuery(connectionString, CommandType.StoredProcedure, DBProc_GetAllotmentSequence, sqlParameters);
                if (sqlParameters[10].Value != null && !string.IsNullOrEmpty(sqlParameters[10].Value.ToString()))
                {
                    return sqlParameters[10].Value.ToString();
                }
                else
                {
                    return string.Empty;
      ...
      protected Dictionary<string, string> SeatRankTypeMapping = null;
        ...
        private void LoadOutputTables()
        {
            ...
            foreach (DataRow dr in dtAllotmentSummary.Rows)
            {
                ...
                if (!SeatRankTypeMapping.ContainsKey(wlKey))
                {
                    throw new Exception(string.Format("Invalid key {0}. Rank type not found for this key", wlKey));
                }
                try
                {
