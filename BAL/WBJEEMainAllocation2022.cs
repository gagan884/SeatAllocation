...
namespace BAL
{
    public class WBJEEMainAllocation2022 : ComCouns21Allocation
    {
        public WBJEEMainAllocation2022() : base()
        {
            // board Id 
            boardId = "134122221";
            ...
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_WBJEEMN";

            //Procedure to prepare Previous allotment or seat gaurnatee candidates
            DBProc_PreparePreviousAllotment = "XApp_CC_PreparePreviousAllotment_WBJEEMN";
        ...
        string[] ArchitecturePrograms = { "11932NN", "11443NN", "11632NN" };
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                while ((rdrChoice.Read()))
                {
                    ...
                    if (isValidChoice)
                    {
                        ...
                        //choiceStreamId = StreamInfoMaster[choiceInstCd + "." + choiceBrCd];//streamId 01,03
                        choiceGroupId = "G1";
                        choiceSeatTypeId = SeatTypeMaster[choiceInstCd + "." + choiceBrCd];//JEEMAIN,WBJEE
                        //choiceGenderId = GenderMaster[choiceInstCd + "." + choiceBrCd];//B

                        if (ArchitecturePrograms.Contains(choiceBrCd) && !candAdditionalInfo.Equals("01:YES"))
                        {
                            continue;
                        }

                        if (choiceSeatTypeId.Contains("JEEMAIN"))
                        {
                            if (ArchitecturePrograms.Contains(choiceBrCd))
                                choiceStreamId = "01";
                            else
                                choiceStreamId = "03";
                            ...
                            foreach (string option in catSubCatOptions.Split(','))
                            {
                                ...
                                waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + ".JEEMAIN." + option + "." + "B";
                        ...
                        if (choiceSeatTypeId.Contains("WBJEE"))
                        {
                            choiceStreamId = "03";
                            foreach (string option in catSubCatOptions.Split(','))
                            {
                                ...
                                waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + ".WBJEE." + option + ".B";
        ...
        {
            bool isArchitecture = ArchitecturePrograms.Contains(programId);
            ...
            if (instituteId.Equals("104637")) //Jadavpur(104637)
            {
                if (isArchitecture) // "11932NN", "11443NN", "11632NN"
                {
                    var class12percentage = GetClass12Percentage(rollno);
                    var percentage_PCM = CalculateAverage(new string[] { "P", "C", "M" }, rollno);
                    if (class12percentage < 50 || percentage_PCM < 50)
                        isEligibleForOpen = "N";

                    if (class12percentage < 37 || percentage_PCM < 37)
                        isEligibleForReserved = "N";
                }
                else if (!programId.Substring(0, 5).Equals("11645")) // other than Marine(11645)
                {
            ...
            else if (instituteId.Equals("104634"))//Sister Nivedita University
            {
                if (programId.Substring(0, 5).Equals("11936")) //11936NN Computer Science & Business System
                {
                    var class12percentage = GetClass12Percentage(rollno);
                    var percentage_PCM = CalculateAverage(new string[] { "P", "C", "M" }, rollno);

                    if (class12percentage < 60 || percentage_PCM < 60)
                        return "NN";
                }
                else if (isArchitecture)  //11632NN B.Arch (General)
                {
                    var class12percentage = GetClass12Percentage(rollno);
                    var percentage_PCM = CalculateAverage(new string[] { "P", "C", "M" }, rollno);
            ...
            //All ,  Architecture("11932NN", "11443NN", "11632NN")
            else if (isArchitecture)
            {
                var class12percentage = GetClass12Percentage(rollno);

                var percentage_PCM = CalculateAverage(new string[] { "P", "C", "M" }, rollno);
                if (percentage_PCM < 50 || class12percentage < 50)
                    isEligibleForOpen = "NN";
            }
            //All ,  Pharma(11633)
