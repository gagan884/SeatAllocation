...
namespace BAL
{
    public class UPTACArchAllocation2022 : ComCouns21Allocation
    {
        public UPTACArchAllocation2022() : base()
        {
            // board Id 
            boardId = "130072221";

            //Procedure to prepare seat
            DBProc_PrepareSeat = "XApp_CC_PrepareSeat_UPSEEAR";

            //Procedure to prepare Eligible Candidate
            DBProc_PrepareEligibleCandidate = "XApp_CC_PrepareEligibleCandidate_UPSEEAR";
            ...
            DBProc_DereserveSeat = "XApp_CC_DereserveSeat_UPSEEAR";
        ...
        public override ActionOutput CreateVirtualChoice()
        {
            ...
            foreach (DataRow drCand in dtCand.Rows)
            {
                ...
                candGendervalue = candGender == "01" ? "M" : "F";
                        ...
                        foreach (string option in catSubCatOptions.Split(','))
                        {
                            foreach (string genderid in choiceGenderId.Split(','))
                            {
                                if (genderid.ToUpper() == candGendervalue.ToUpper() || genderid.ToUpper() == "B")
                                {
                                    ...
                                    waitListKey = choiceInstCd + "." + choiceBrCd + "." + choiceStreamId + "." + choiceGroupId + "." + choiceSeatTypeId + "." + option + "." + genderid;
